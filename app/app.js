...
function CreateGrid(cols, result, columngroups) {
    ...
    for (var y = 0; y < cols.length; y++) {
        datastructure.push({ name: cols[y]['name'], map: cols[y]['map'], type: 'number'});
    }
    var source =
    {
        localdata: result,
    ...
    var plcolumns = [];
    plcolumns.push({ text:'Year', datafield: 'item', align: 'left', cellsalign: 'left', width: '100px', editable: false });
    for (var y = 0; y < cols.length; y++) {
        plcolumns.push({
            text: cols[y]['text'], datafield: cols[y]['name'], cellsalign: 'right', align: 'right', columngroup:cols[y]['columngroup'], editable:cols[y]['editable'], cellsformat: decimal, 
...
tabs['plant']=['plant_general','plant_production', 'plant_omcosts', 'plant_fuelcosts','plant_generalexpenses','plant_investments','plant_depdec', 'plant_sources'];
...
translates['plant']=['Plant data', 'Plant production', 'Operation & Maintenance costs', 'Fuel cost information', 'General expenses data','Investment cost in constant prices','Depreciation and decommissioning', 'Source of financing']
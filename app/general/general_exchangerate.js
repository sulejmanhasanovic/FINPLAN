...
function showData(results) {
    ...
    for(var j=0; j<curTypeSel.length; j++){
        ...
        var editable=true;
        var cellclassname="";
        if(cedata["RateType"+curTypeSel[j]]=="SR"){ checkedSR="checked"; editable=false; cellclassname="readonly";  }
        
        var checkedII="";
        if(cedata["RateType"+curTypeSel[j]]=="II"){ checkedII="checked"; editable=false; cellclassname="readonly"; disabled="disabled"; }
        
        var disabled="";
        var checkedYR="";
        if(cedata["RateType"+curTypeSel[j]]=="YR"){ checkedYR="checked"; disabled="disabled" }
        ...
        <div class='row'> \
         ...
         <div class='col-md-6'>\
             <input id='YearlyRate"+curTypeSel[j]+"' type='text' class='form-control' size='50' autocomplete='off' value='"+yearlyrate+"' "+disabled+"/> \
         </div> \
         </div> \
         <span class='pure-radiobutton'> \
        ...
        <span class='pure-radiobutton'> \
         ...
         cols.push({name:curTypeSel[j], editable:editable, cellclassname:cellclassname,  map: curTypeSel[j], text:baseCurrencyName +'('+currencyName+')'});
    }

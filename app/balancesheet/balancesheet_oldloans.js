...
for (var i=startYear; i<=endYear; i++){
            ...
            // data['L_'+baseCurrency]=ctdata['L_'+baseCurrency+'_'+i];
...
function showData(results) {
    ...
    for(var j=0; j<bothCurr.length; j++){
        ...
        <div class='row'> \
         ...
         <div class='col-md-6'>\
         <input id='OL_"+bothCurr[j]+"' type='text' class='form-control' size='50' autocomplete='off' value='"+ol+"'/> \
         </div> \
         </div> \
        <div class='row'> \
         <div class='col-md-6'> \
        ...
        cols.push({name:'L_'+bothCurr[j], columngroup:currencyName, editable:true, map: 'L_'+bothCurr[j], text:'Committed Drawdown'});
        cols.push({name:'R_'+bothCurr[j], columngroup:currencyName, editable:true, map: 'R_'+bothCurr[j], text:'Repayment'});
        cols.push({name:'I_'+bothCurr[j], columngroup:currencyName, editable:editable, cellclassname:cellclassname, map: 'I_'+bothCurr[j], text:'Investment'});
    }

    // var ol=ctdata["OL_"+baseCurrency];
    // if(ol===undefined)
    //     ol="";
    ...
    //     </div> \
    //     <div class='col-md-6'>\
    //     <input id='OL_"+baseCurrency+"' type='text' class='form-control' size='50' autocomplete='off' value='"+ol+"'/> \
    //     </div> \
    //     </div> \
    //     <div class='row'> \
    //         <div class='col-md-6'> \
    //             <span class='pure-radiobutton'> \
    //                 <input type='radio' id='SR"+baseCurrency+"' name='RateType"+baseCurrency+"' value='RS' "+checkedSR+"/><label for='SR"+baseCurrency+"'>Average Return Rate</label> \
    //             </span> \
    //         </div> \
    //         <div class='col-md-6'>\
    ...
    //     </div> \
    //     <span class='pure-radiobutton'> \
    //         <input type='radio' id='YR"+baseCurrency+"' name='RateType"+baseCurrency+"' value='YR' "+checkedYR+"/><label for='YR"+baseCurrency+"'>Yearly Return Payment</label> \
    //     </span> \
    //     </td></tr>";
        $("#controls").append(tblcontrols);

        // cols.push({name:'L_'+baseCurrency, editable:true, map: 'L_'+baseCurrency, text:'L_'+baseCurrencyName});
        // cols.push({name:'R_'+baseCurrency, editable:true, map: 'R_'+baseCurrency, text:'R_'+baseCurrencyName});
        // cols.push({name:'I_'+baseCurrency, editable:editable, cellclassname:cellclassname, map: 'I_'+baseCurrency, text:'I_'+baseCurrencyName});

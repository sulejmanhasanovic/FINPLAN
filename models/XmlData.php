...
class XmlData extends Base {
	...
	public function __construct($caseStudyId, $objectName){
		...
		$dir = PROJECT_DATA_FILE_PATH.sanitize_filename($this->caseStudyId);
		if (!is_dir($dir)) {
		mkdir($dir,0777);
		}
		if($filechunk[0] == 'cal'){//added for result folder
			$this->xmlFile =  PROJECT_DATA_FILE_PATH.sanitize_filename($this->caseStudyId).'/result/'.$this->objectName.'.'.DATA_FILE_EXT;
		}
		else{
			$this->xmlFile =  PROJECT_DATA_FILE_PATH.sanitize_filename($this->caseStudyId).'/'.$this->objectName.'.'.DATA_FILE_EXT;
		}

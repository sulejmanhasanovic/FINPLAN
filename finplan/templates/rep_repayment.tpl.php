...
<h3> Repayment for each plant: currency wise</h3>
<table>
  <tr>
...
<br>
<h3>Plant : <?php echo $cpData['name'];?>&nbsp; Currency :<?php echo Config2::getData('currencies',$_REQUEST['cid']);?></h3>
...
?>
    <td valign="top"><table border="1" width="<?php echo $tablewidth;?>" cellpadding="0" cellspacing="0">
        ...
        <?php		for($m=$ahData['startYear'];$m <= $ahData['endYear']; $m++){
			$ddval = 'Repy_'.$_REQUEST['cid'].'_'.$financesource['id'].'_'.$m;
?>
        <tr>
          <td><?php echo $ceData[$ddval];?></td>
        </tr>
        <?php		} 
		$totddval = 'TotREP_'.$_REQUEST['cid'].'_'.$financesource['id'];
?>
        <tr>
